...
- [ ] 支持 `.pysecrc` YAML/TOML 配置文件
- [ ] 支持 `pyproject.toml` 中的 `[tool.pysec]` 配置节
- [ ] 配置项包括：
  - 启用/禁用的规则列表
  - 排除的目录和文件模式
  - 自定义严重程度阈值
...
- [ ] 支持指定基准提交/分支进行比较
- [ ] 缓存 AST 解析结果，加速重复扫描
- [ ] 实现 `--changed-only` 和 `--since <commit>` 参数
...
- [ ] 检测 `marshal.loads()` 不安全反序列化
- [ ] 检测 `shelve.open()` 不安全使用
- [ ] 检测 `jsonpickle.decode()` 风险
- [ ] 检测 `dill.loads()` 风险
- [ ] 提供安全的替代方案建议
...
| Phase                 | 完成度 | 状态      |
| --------------------- | ------ | --------- |
| Phase 1: 核心功能增强 | 0%     | ⏳ 待开始 |
| Phase 2: 新增检测规则 | 0%     | ⏳ 待开始 |
| Phase 3: 报告与可视化 | 30%    | 🔄 进行中 |
| Phase 4: 集成与自动化 | 0%     | ⏳ 待开始 |
| Phase 5: 性能优化     | 0%     | ⏳ 待开始 |
| Phase 6: 文档与生态   | 20%    | 🔄 进行中 |
